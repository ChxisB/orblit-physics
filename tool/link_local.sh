#!/bin/bash
# Points orblit_physics_scene at a sibling engine checkout instead of the git
# dependency, so a change to the scene format can be tried here without pushing
# it. The override is not committed, so nobody ships a build wired to a local
# path. orblit_physics itself depends on nothing and needs no linking.
set -euo pipefail
cd "$(dirname "$0")/.."
